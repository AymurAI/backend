...
from ..utils import format_entity, dataset_to_docbin

logger = get_logger(__name__)


class SpacyNER(TrainModule):
    CACHE_PATH = "/resources/cache/spacy"
    BASE_PATH = resolve_package_path("aymurai.spacy.configs")

    def __init__(
        self,
        base_config: Union[str, Path] = "ner-transformer",
        ...
        if (  # read config
            ...
            or Path(base_config := f"{self.BASE_PATH}/{base_config}.cfg").is_file()
        ):
            ...
            # __nlp = spacy.blank(lang).load_config(config)
    ...
    def load(cls, path: str):
        ...
        return cls(base_model=path)

    def fit(self, train: DataBlock, val: DataBlock):
        logger.info("building train docbin")
        train_path = dataset_to_docbin(self.nlp, train, use_cache=self.dataset_cache)
        logger.info("building val docbin")
        val_path = dataset_to_docbin(self.nlp, val, use_cache=self.dataset_cache)
    ...
    def predict(self, data: DataBlock) -> DataBlock:
        ...
        ents = (map(_format_entity, doc.ents) for doc in docs)
        for item, ents in zip(data, ents):
    ...
    def single_predict(self, item: DataItem) -> DataItem:
        ...
        ents = map(_format_entity, doc.ents)
        item["data"]["entities"] = list(ents)
