...
class ArgentinaJuzgadoPCyF10PDFDataset(UserList):
    ...
    def get_registro(path: str) -> dict:
        ...
        match = re.findall(r"/(\d{4})/", dirname)
    ...
    def __init__(self, use_cache: Union[str, bool] = True):
        PDFS_PATH = os.environ["AYMURAI_RESTRICTED_DOCUMENT_PDFS_PATH"]
        cache_overwrite = use_cache == "overwrite"

        cache_key = get_cache_key("private-pdf", context="dataset-ar-juz-pcyf-10")
        if (
            use_cache
        ...
        # load file paths
        paths = iglob(f"{PDFS_PATH}/**/*.*", recursive=True)
        paths = filter(os.path.isfile, paths)
        paths = list(sorted(paths))
        # print(paths)

        docs = pd.DataFrame([self.get_registro(path) for path in paths])

        docs.dropna(subset=["nro_registro", "tomo", "year"], inplace=True)
        docs["nro_registro"] = docs["nro_registro"].astype(str)
        docs["year"] = docs["year"].astype("int32")
        docs["tomo"] = docs["tomo"].astype("int32")

        annotations = annotations.merge(docs, on=["nro_registro", "tomo"])
