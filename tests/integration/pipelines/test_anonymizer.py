...
def _extract_entities(item: dict[str, Any]) -> list[dict[str, Any]]:
    predictions = item.get("predictions")
    if isinstance(predictions, dict):
        entities = predictions.get("entities") or []
...
def test_should_predict_single_when_given_preprocessed_item(
    ...
    entities = _extract_entities(predicted_item)
    assert entities, "Expected at least one anonymizer entity in canonical sample text"
    first_entity = entities[0]
    assert first_entity.get("text")
    assert isinstance(first_entity.get("start_char"), int)
    assert isinstance(first_entity.get("end_char"), int)
    assert isinstance(first_entity.get("attrs"), dict)
...
def test_should_produce_anonymization_entities_when_running_full_chain(
    ...
    assert entities, "Expected postprocess to preserve anonymizer entities"
