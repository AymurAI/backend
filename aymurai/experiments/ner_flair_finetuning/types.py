...
BackendMode = Literal["pretrained_flair", "base_flair"]
...
class CanonicalSample:
    """A sample containing text and its gold standard entities."""

    sample_id: str
    text: str
    tokens: list[str] = field(default_factory=list)
    gold_spans: list[CanonicalSpan] = field(default_factory=list)
    gold_bio: Optional[list[str]] = None
    metadata: dict[str, Any] = field(default_factory=dict)


@dataclass
class BackendPrediction:
    """Output from the Flair model for comparison."""
