...
def _resolve_model_name(config: NERTestsetEvaluationConfig) -> str:
...
def _ensure_dirs(config: NERTestsetEvaluationConfig) -> dict[str, Path]:
...
def run_experiment(config_path: str) -> None:
    ...
    predictions, traces, sample_trace_ids = run_backend_inference(samples, config)
    ...
    feedback_records, _ = build_feedback_records(
        sample_scores,
    ...
    write_jsonl(paths["samples_gold"], [sample_to_dict(sample) for sample in samples])
    ...
    write_label_metrics_csv(paths["strict_label_metrics"], summary.strict_label_metrics)
    write_csv(
    ...
    with run_context:
        if mlflow_enabled:
            mlflow.set_tags(
                {
                    ...
                    "pipeline": "ner_testset_evaluation",
                    "backend_mode": config.backend.mode,
                }
            )
            ...
            log_feedback_to_mlflow(
                enabled=mlflow_enabled, feedback_records=feedback_records
            )
...
def build_arg_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run NER/LangExtract test set evaluation experiment from YAML config."
    )
    parser.add_argument(
