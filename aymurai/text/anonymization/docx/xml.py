...
from aymurai.text.anonymization.alignment import parse_token_indices
...
def replace_text_in_xml(
    paragraphs: list[dict], base_dir: str, render_context: dict | None = None
) -> None:
    """
    ...
    """
    tokens = pd.concat(
        [parse_token_indices(sample, render_context) for sample in paragraphs],
        ignore_index=True,
    )
...
def create_docx(xml_directory: str, output_file: str) -> None:
    """
